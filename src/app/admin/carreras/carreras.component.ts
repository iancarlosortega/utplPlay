    ...
    if (this.carrera.id) {
      ...
      if (this.selectedFiles) {
        ...
        if (file) {
          ...
          this.adminService
            .agregarCarrera(this.currentFileUpload, this.carrera, this.tipo)
            .subscribe((percentage) => {
              this.percentage = Math.round(percentage ? percentage : 0);
              if (this.percentage == 100) {
                setTimeout(() => {
                  this.toastr.info(
                    'La carrera fue actualizada con éxito!',
                    'Carrera Actualizada'
                  );
                  this.enableForm();
                  this.url = null;
          ...
          this.adminService
            ...
            .subscribe((percentage) => {
              ...
              if (this.percentage == 100) {
                setTimeout(() => {
                  this.toastr.success(
                    `La carrera ${this.carrera.name} fue registrada con éxito!`,
                    'Carrera Registrada'
                  );
                  this.enableForm();
                  this.miFormulario.reset();
    ...
    dialog.afterClosed().subscribe((result) => {
      if (result) {
        this.adminService
          ...
          .then((res) => {
            this.toastr.success(
              'La carrera fue eliminada con éxito',
              'Carrera eliminada!'
            );
          })
          .catch((err) => {
